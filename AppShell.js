...
const cardBase = (brand) => ({ background: brand.paper, color: brand.ink, borderRadius: 16, boxShadow: "0 10px 30px rgba(0,0,0,0.15)" });
...
export default function AppShell() {
  ...
  const NavLink = ({ id, label }) => (
    <button
      ...
      className={`px-4 py-2 rounded-full text-sm font-medium transition-all duration-200 ${screen==='home' && id==='home' || screen===id ? 'bg-white text-black shadow' : 'bg-black/30 text-white hover:bg-black/50'}`}
    ...
    <div className="relative overflow-hidden" style={cardBase(brand)}>
      <div className="grid md:grid-cols-2">
        <div className="p-8 md:p-12 flex flex-col gap-4">
          ...
          <p className="text-sm md:text-base text-black/70">Bespoke holidays, cruise journeys and city breaks. Book with confidence via our approved supplier portals and enjoy ABTA & ATOL protection where applicable.</p>
...
function Tag({ label, active, onToggle }) {
  return (
    <button type="button" onClick={onToggle} className={`px-3 py-1 rounded-full text-xs border ${active ? '' : 'bg-white'}`}
...
function QuoteForm({ onSubmitted }) {
  const [form, setForm] = useState({
    name: "", email: "", phone: "", dates: "", adults: 2, children: 0, budget: "", style: "Luxury", interests: ["City"]
  });
        ...
        <Field label="Interests" helper="Select all that apply">
          <div className="flex flex-wrap gap-2">
            {"City Beach Cruise Nature Food Wine Culture Theme Parks".split(" ").map((tag) => (
              <Tag key={tag} label={tag} active={form.interests.includes(tag)} onToggle={() => setForm((f) => ({ ...f, interests: f.interests.includes(tag) ? f.interests.filter((t)=>t!==tag) : [...f.interests, tag] }))} />
            ))}
          </div>
        </Field>
...
function Detail({ label, value }) { 
  return <div className="flex items-center justify-between py-2 border-b last:border-b-0" style={{ borderColor: "#eee" }}>
    <div className="text-black/60">{label}</div><div className="font-medium text-black/80">{value}</div>
  </div>;
}
      ...
      <div className="p-6 md:p-8 grid gap-4">
        {items.map((it) => (
          <div key={it.ref} className="flex items-center justify-between p-4 rounded-xl bg-white border" style={{ borderColor: brand.stone }}>
            <div><div className="text-xs font-medium" style={{ color: brand.green }}>{it.type}</div><div className="font-semibold text-black/80">{it.title}</div><div className="text-sm text-black/60">{it.time}</div></div>
...
function ComplianceFooter() {
  return (
    <footer className="p-5 text-[12px] rounded-2xl" style={{ background: "rgba(255,255,255,0.06)", color: "#E8E8E8" }}>
      <p>LEXVOYAGE LTD acts as an Independent Travel Agent of InteleTravel UK. All bookings and payments are made via approved supplier portals or the InteleTravel back office. ABTA & ATOL protection apply where applicable. Terms & conditions from the respective supplier apply.</p>
    </footer>
  );
}
